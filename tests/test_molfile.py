...
class DtrTestCase(unittest.TestCase):
    ...
    def testCheckpointBytes(self):
        ...
        self.assertEqual(
            kv["ANTON2_CHECKPOINT_CM_TIMES_SQUARE"][:16],
            b"\x1d\x00\x00\x00Times square",
        )
    ...
    def testBadTimekeys(self):
        with open("%s/timekeys" % self.PATH, "w") as fd:
            fd.write("hello")
        with self.assertRaises(RuntimeError):
            molfile.DtrReader(self.PATH)
...
class TestFrameCornerCases(unittest.TestCase):
    ...
    def testKeyvals(self):
        ...
        for i in range(dtr.nframes):
            ...
            self.assertEqual(kv["gid__alchemical_angle_harm"].tolist(), want)
...
class TestDtrWriterEtr(unittest.TestCase):
    ...
    def testEtrFormat(self):
        ...
        assert kv1["Y"][0] == 1.0
        assert sorted(reader.metadata.keys()) == sorted(
            ["FORMAT"] + list(keyvals.keys())
        )
...
class TestQuantizedTime(unittest.TestCase):
    ...
    # too slow...   ..................................  \/
    @unittest.skipIf(
        os.getenv("DESRES_LOCATION") != "EN_XXX", "Runs only from EN location"
    )
